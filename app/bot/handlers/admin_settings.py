...
async def _render_chat(update: Update, context: ContextTypes.DEFAULT_TYPE, chat_id: int, language: str) -> None:
    repo = _repo(context)
    if repo is None:
        return
    ...
    markup = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("Timezone", callback_data=f"aset_tz_{chat_id}"), InlineKeyboardButton("Close", callback_data="aset_exit")],
    ])
...
async def _render_chat_list(update: Update, context: ContextTypes.DEFAULT_TYPE, language: str) -> None:
    repo = _repo(context)
    user = update.effective_user
    ...
    if not chats:
        ...
        return
    keyboard = [
        [InlineKeyboardButton(f"👥 {chat.chat_id} ({chat.chat_type})", callback_data=f"aset_menu_{chat.chat_id}")]
        for chat in chats
    ]
...
async def admin_settings_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if chat.type == "private":
        ...
        return
    if update.effective_message:
        await update.effective_message.reply_text("Use /admin_settings in a group or private chat.")
...
async def admin_settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "toggle":
        ...
        markup = InlineKeyboardMarkup([[InlineKeyboardButton(f"{hour:02d}:00", callback_data=f"aset_settime_{chat_id}_{hour:02d}:00")] for hour in range(24)])
        await _safe_edit(query, "Select delivery time:", markup)
        return
    elif action == "settime" and len(parts) == 4:
        if len(parts[3]) != 5 or parts[3][2] != ":" or not all(part.isdigit() for part in parts[3].split(":")):
        ...
        if cities is None:
            return
        await _safe_edit(query, "Select city:", InlineKeyboardMarkup([[InlineKeyboardButton(city, callback_data=f"aset_settz_{chat_id}_{city}")] for city in cities]))
        return
