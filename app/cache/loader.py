...
class QuranCacheLoader:
    """Load Quran resources while preventing overlapping reloads."""

    def __init__(self, provider: NatiqProvider, cache: QuranCache) -> None:
    ...
    async def load_detailed(self) -> tuple[bool, dict[str, str]]:
        async with self._load_lock:
            return await self._load_detailed_locked()

    async def _load_detailed_locked(self) -> tuple[bool, dict[str, str]]:
