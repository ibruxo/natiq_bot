...
class ChatRepository:
    ...
    async def get_or_create(self, *, telegram_id: int, chat_type: str = ChatType.PRIVATE.value, language: str = "fa", enable_daily_ayah: bool = True) -> "Chat":
        from app.database.models.chat import Chat
        ...
        async with self._database.session() as session:
            ...
            if chat is None:
                raise RuntimeError(f"Chat was not available after upsert: {telegram_id}")
    ...
    async def is_admin_for_chat(self, chat_id: int, user_id: int) -> bool:
        ...
        async with self._database.session() as session:
            return bool(await session.scalar(select(ChatAdmin.uuid).where(ChatAdmin.chat_id == chat_id, ChatAdmin.admin_telegram_id == user_id)))
    ...
    async def update_preferences(self, telegram_id: int, **preferences: object) -> "Chat | None":
        allowed = {"language", "daily_ayah", "daily_time", "timezone", "content_mode", "daily_type", "delivery_mode"}
    ...
    async def add_chat_admin(self, chat_id: int, chat_type: str, admin_id: int) -> None:
        from app.database.models.chat_admin import ChatAdmin
        async with self._database.session() as session:
            await session.execute(insert(ChatAdmin).values(chat_id=chat_id, chat_type=chat_type, admin_telegram_id=admin_id).on_conflict_do_nothing(constraint="uq_chat_admin_chat_user"))
            await session.commit()
    ...
    async def save_chat_admins(self, chat_id: int, chat_type: str, admin_ids: list[int]) -> None:
        ...
        async with self._database.session() as session:
            ...
            if unique_ids:
                await session.execute(insert(ChatAdmin), [{"chat_id": chat_id, "chat_type": chat_type, "admin_telegram_id": admin_id} for admin_id in unique_ids])
            await session.commit()
    ...
    async def list_chats_administered_by(self, user_id: int) -> list["Chat"]:
        ...
        from app.database.models.chat_admin import ChatAdmin
        async with self._database.session() as session:
            result = await session.execute(select(Chat).join(ChatAdmin, Chat.chat_id == ChatAdmin.chat_id).where(ChatAdmin.admin_telegram_id == user_id))
            return list(result.scalars().unique().all())

    async def list_group_chats(self) -> list["Chat"]:
        from app.database.models.chat import Chat
        async with self._database.session() as session:
            result = await session.execute(select(Chat).where(Chat.chat_type.in_((ChatType.GROUP.value, ChatType.SUPERGROUP.value, ChatType.CHANNEL.value))))
    ...
    async def should_send_daily_ayah(self, telegram_id: int) -> bool:
        from app.database.models.chat import Chat
        async with self._database.session() as session:
            result = await session.execute(select(Chat).where(Chat.chat_id == telegram_id).options(load_only(Chat.daily_ayah, Chat.last_daily_sent_date, Chat.timezone)))
    ...
    async def mark_daily_ayah_sent(self, telegram_id: int) -> None:
        from app.database.models.chat import Chat
        async with self._database.session() as session:
            today = await session.scalar(select(Chat.timezone).where(Chat.chat_id == telegram_id))
            if today is not None:
                await session.execute(__import__("sqlalchemy").update(Chat).where(Chat.chat_id == telegram_id).values(last_daily_sent_date=self._local_today(today)))
                await session.commit()
    ...
    async def get_send_totals(self) -> dict[str, int]:
        ...
        return {"ayahs": sum(int(c) for mode, c in rows if mode == ReadingMode.AYAH), "pages": sum(int(c) for mode, c in rows if mode == ReadingMode.PAGE)}
