...
class SentHistoryRepository:
    ...
    async def log_sent(
        self, *, chat_uuid: uuid.UUID, ayah_uuid: uuid.UUID, reading_mode: str = "ayah"
    ) -> "SentHistory":
        from app.database.models.sent_history import ReadingMode, SentHistory

        history = SentHistory(
            chat_uuid=chat_uuid, ayah_uuid=ayah_uuid, type=ReadingMode(reading_mode)
        )
    ...
    async def get_total_sent_counts(self) -> dict[str, int]:
        ...
        return {
            "ayahs": sum(
                int(count) for mode, count in rows if mode == ReadingMode.AYAH
            ),
            "pages": sum(
                int(count) for mode, count in rows if mode == ReadingMode.PAGE
            ),
        }
