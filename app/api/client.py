...
class APIClient:
    def __init__(self) -> None:
        ...
        self._secondary_client = (
            ...
            if secondary and secondary.rstrip("/") != self._settings.NATIQ_PRIMARY_API.rstrip("/")
            else None
        )
    ...
    async def _request(self, method: str, endpoint: str, *, params: dict[str, Any] | None = None, json: dict[str, Any] | None = None) -> httpx.Response:
        ...
        try:
            ...
            response = await self._secondary_client.request(method, endpoint, params=params, json=json)
        if response.is_error:
            logger.warning("Natiq API %s %s returned HTTP %s", method, endpoint, response.status_code)
    ...
    async def put(self, endpoint: str, *, json: dict[str, Any] | None = None) -> httpx.Response:
        return await self._request("PUT", endpoint, json=json)

    async def patch(self, endpoint: str, *, json: dict[str, Any] | None = None) -> httpx.Response:
